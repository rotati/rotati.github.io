...
class XMLValidator < W3CValidators::Validator
    def validate_file(file)
        ...
        begin
            ...
            @results.add_message(:error, {message: 'Nokogiri threw errors on input.'})
        end
        @results
    end
end
...
class HtmlValidator < W3CValidators::Validator
    def validate_file(file)
        ...
        validator.errors.each { |err| @results.add_message(:error, err) }
        @results
    end
end
...
Dir.glob("_site/index.html") do |file|
